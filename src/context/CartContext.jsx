...
export default function CartProvider({ children }) {
  ...
  const [cartItems, setCartItems] = useState([]); // {id: 2, quantity: 1}

  function addToCart(productId) {
    const existingItem = cartItems.find((item) => item.id === productId);

    if (existingItem) {
      const currQuantity = existingItem.quantity;
      const updatedCartItems = cartItems.map(item => item.id === productId ? { id: productId, quantity: currQuantity + 1 } : item);
